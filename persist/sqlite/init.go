...
func initializeSettings(ctx context.Context, tx *txn, target int64) error {
	_, err := tx.Exec(ctx, `INSERT INTO global_settings (id, db_version) VALUES (0, ?)`, target)
	return err
}
...
func (s *Store) initNewDatabase(ctx context.Context, target int64) error {
	return s.transaction(ctx, func(ctx context.Context, tx *txn) error {
		...
		} else if err := initializeSettings(ctx, tx, target); err != nil {
...
func (s *Store) upgradeDatabase(ctx context.Context, current, target int64) error {
	...
	for ; current < target; current++ {
		...
		err := s.transaction(ctx, func(ctx context.Context, tx *txn) error {
			...
			} else if err := fn(ctx, tx, log); err != nil {
				return err
			} else if err := foreignKeyCheck(ctx, tx, log); err != nil {
...
func (s *Store) init(ctx context.Context) error {
	...
	switch {
	case version == 0:
		return s.initNewDatabase(ctx, target)
	case version < target:
		return s.upgradeDatabase(ctx, version, target)
	case version > target:
...
func foreignKeyCheck(ctx context.Context, txn *txn, log *zap.Logger) error {
	rows, err := txn.QueryContext(ctx, "PRAGMA foreign_key_check")
	if err != nil {
